 ...
 */
export const comparePasswordController = async (req: Request, res: Response): Promise<void> => {
  try {
    const compare = await comparePasswordService(req.body.password, req.body.hashPassword);
    res.status(200).send({ message: "Contraseña comparada correctamente", value: compare });
  } catch (error) {
